...
<section <?php ign_block_attrs( $block, 'text-image-section full-width' ); ?> style="background-color:<?php echo esc_attr( $background_color ); ?>" data-scroll-section>
	...
	<div class="text-image-container <?php echo esc_attr( $text_position [0] ); ?> <?php echo esc_attr( $text_spacing [0] ); ?>">
		...
		<div class="block-text-container" style="background-color:<?php echo esc_attr( $background_color ); ?>;justify-content:<?php echo esc_attr( $vertical_align_text[0] ); ?>" data-scroll data-scroll-repeat data-scroll-speed="4" data-scroll-offset="300px"><?php the_field( 'text' ); ?></div>
		<?php endif; ?>
		<?php
		if ( $text_type[0] === 'icon' ) :
			?>
