...
function ign_menu( $item, $args ) {
	...
	if ( in_array( 'menu-item-has-children', (array) $classes ) ) {
		$item .= '<button tabindex="-1" class="submenu-dropdown-toggle">' . $arrow . '
                    <span class="screen-reader-text">' . __( 'Expand child menu', 'dixeed-2023' ) . '</span></button>';
	}
 ...
 * @return void
 */
function dixeed_2023_output_inline_svg( $html ) {
...
add_filter( 'get_custom_logo', 'dixeed_2023_output_inline_svg' );
 ...
 *
 */
function dixeed_2023_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'dixeed-2023-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'wp_resource_hints', 'dixeed_2023_resource_hints', 10, 2 );
